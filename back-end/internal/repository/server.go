...
func (ss *ServerRepository) GetServersByOptionalDateRange(startCreated, endCreated, startUpdated, endUpdated time.Time, field, order string) ([]model.Server, error) {
	query := ss.DB
...
func (ss *ServerRepository) Create(s *model.Server) error {
	return ss.DB.Create(s).Error
}

// CreateMany creates multiple servers in the database.
func (ss *ServerRepository) CreateMany(servers []model.Server) error {
...
func (ss *ServerRepository) Delete(id string) error {
